...
def main():
    ...
    predict_svr = Training_Prediction_ML(model_SVR(), a, scaler, data_regular, 'SVR')
    predict_elm = Training_Prediction_ML(model_ELM(), a, scaler, data_regular, 'ELM')
    predict_bp = Training_Prediction_ML(model_BPNN(), a, scaler, data_regular, 'BPNN')

    predict_LSTM = Training_Prediction_DL(model_LSTM(lookback_window), a, scaler, data_regular_DL, 'LSTM')
    #################################################################EMD_LSTM
    ...
    for emd_imf in emd_imfs:
        ...
        emd_imfs_prediction.append(np.concatenate((emd_prediction_X, emd_prediction_Y), axis=0))
    ...
    # plt.plot(eemd_prediction, 'r', linewidth=2.5, linestyle='--', marker='^', markersize=2)
    plt.plot(ceemdan_prediction, 'red',  linewidth=1, linestyle='-', marker='^', markersize=2)
    plt.grid(True, linestyle=':', color='gray', linewidth='0.5', axis='both')
    plt.xlabel('time(days)', fontsize=18)
    plt.ylabel('height(mm)', fontsize=18)
    # plt.title('551')
